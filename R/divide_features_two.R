...
divide_features_two <- function(n_features_two, num.factor) {
  ...
  if (num * min_size >= n_features_two) {
  ...
  remaining_length <- n_features_two - num * min_size
  ...
  segment_sizes <- diff(c(0, breakpoints, n_features_two))

  # Ensure all segment sizes are greater than min_size
  if (any(segment_sizes <= min_size)) {
    return(divide_vector(n_features_two, num, min_size)) # Retry if any segment is less than or equal to min_size
  }
